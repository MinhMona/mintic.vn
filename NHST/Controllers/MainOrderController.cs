...
namespace NHST.Controllers
{
    public class MainOrderController
    {
        ...
        public static string UpdateFeeScanKho(int ID, string FeeWeight, double UnitPrice, string TotalPriceVND)
        {
            using (var dbe = new NHSTEntities())
            {
                var or = dbe.tbl_MainOder.Where(o => o.ID == ID).FirstOrDefault();
        ...
        public static string UpdateUnitPrice(int ID, double UnitPrice)
        {
            using (var dbe = new NHSTEntities())
            {
                var or = dbe.tbl_MainOder.Where(o => o.ID == ID).FirstOrDefault();
        ...
        public static List<mainorder> GetAllByUIDNotHidden_SqlHelperNew(int UID, string search, int typesearch, int status, string fd, string td, int OrderType, int page)
        {
            ...
            sql += " Order By mo.ID desc OFFSET (" + page + "*50) ROWS FETCH NEXT 20 ROWS ONLY ";
            var reader = (IDataReader)SqlHelper.ExecuteDataReader(sql);
            int i = 1;
            while (reader.Read())
            {

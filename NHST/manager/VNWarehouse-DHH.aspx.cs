...
using static NHST.manager.TQWareHouse_DHH;
...
namespace NHST.manager
{
    public partial class VNWarehouse_DHH : System.Web.UI.Page
    {
            ...
            if (package != null)
            {
                ...
                else
                {
                    ...
                    if (mainorder != null)
                    {
                        ...
                        int LevelID = Convert.ToInt32(usercreate.LevelID);
                        ...
                        if (smallpackage.Count > 0)
                        {
                            ...
                            {
                                var fee = FeeOrderController.GetAllMainOrder(warehouseFrom, warehouse, shipping, LevelID);
                                if (fee != null)
                                {
                                    pricePerWeight = Convert.ToDouble(fee.Price);
                                    returnprice = totalWeight * pricePerWeight;
                                }
                            }
                        ...
                        MainOrderController.UpdateFeeScanKho(orderID, FeeWeight.ToString(), pricePerWeight, TotalPriceVND.ToString());
                        ...
                        if (setNoti != null)
                        {
                            ...
                            if (setNoti.IsSendEmailUser == true)
                            {
                                try
                                {
                                    PJUtils.SendMailGmail("admin@mintic.vn", "monamedia", usercreate.Email,
                                    "Thông báo tại Mintic Logistics.", "Hàng của đơn hàng " + mainorder.ID + " Hàng về kho VN.", "");
                                }
                                catch { }
                            }
                        }
                    ...
                    //                                    acc.Email, "Thông báo tại Mintic Logistics.",
        ...
        [WebMethod]
        public static string PriceCard(string barcode)
        {
            if (HttpContext.Current.Session["userLoginSystem"] != null)
            {
                string username = HttpContext.Current.Session["userLoginSystem"].ToString();
                ...
                if (user != null)
                {
                    ...
                    if (userRole == 0 || userRole == 2 || userRole == 4)
                    {
                        if (!string.IsNullOrEmpty(barcode))
                        {
                            CardInfo b = new CardInfo();
                            string Username = "";
                            b.Weight = string.Empty;
                            var sm = SmallPackageController.GetByOrderTransactionCode(barcode);
                            if (sm != null)
                            {
                                b.Quantity = sm.Quantity.ToString();
                                double length = Convert.ToDouble(sm.Length);
                                double width = Convert.ToDouble(sm.Width);
                                double height = Convert.ToDouble(sm.Height);
                                double quiDoi = 0;
                                if (length > 0 && width > 0 && height > 0)
                                    quiDoi = (length * width * height) / 6000;
                                double thucte = sm.Weight ?? 0;
                                if (quiDoi > thucte)
                                    b.Weight = quiDoi.ToString();
                                else
                                    b.Weight = thucte.ToString();
                                if (Convert.ToInt32(sm.MainOrderID) > 0)
                                {
                                    b.OrderID = sm.MainOrderID.ToString();
                                    var main = MainOrderController.GetAllByID(Convert.ToInt32(sm.MainOrderID));
                                    if (main != null)
                                    {
                                        if (main.FromPlace != null)
                                        {
                                            var warehouse = WarehouseController.GetByID(Convert.ToInt32(main.ReceivePlace));
                                            b.Warehouse = warehouse.WareHouseName;
                                        }
                                        ...
                                        var od = OrderController.GetByMainOrderID(main.ID);
                                        int soLuong = 0;
                                        if (od.Count > 0)
                                        {
                                            List<Order> lo = new List<Order>();
                                            foreach (var temp in od)
                                            {
                                                soLuong += temp.quantity.ToInt();
                                            }
                                        }

                                        b.Quantity = soLuong.ToString();
                                    }
                                }
                                else if (Convert.ToInt32(sm.TransportationOrderID) > 0)
                                {
                                    b.OrderID = sm.TransportationOrderID.ToString();
                                    var trans = TransportationOrderController.GetByID(Convert.ToInt32(sm.TransportationOrderID));
                                    if (trans != null)
                                    {
                                        var ac = AccountController.GetByID(trans.UID.Value);
                            ...
                            //string barcodeIMG = "/Uploads/smallpackagebarcode/" + barcode + ".Png";
                            //System.Drawing.Image barCode = PJUtils.MakeCardImage(barcode, 2, true);
                            //barCode.Save(HttpContext.Current.Server.MapPath("" + barcodeIMG + ""), ImageFormat.Png);
                            b.Barcode = barcode;
                            JavaScriptSerializer serializer = new JavaScriptSerializer();
                            return serializer.Serialize(b);

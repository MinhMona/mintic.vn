    ...
    public partial class Report_Sales : System.Web.UI.Page
    {
        ...
        private void LoadData()
        {
            ...
            if (ac != null)
            {
                ...
                var la = MainOrderController.OrderListSalesStore(ac.ID, stype, search, fd, td, 20, page);
                var total = 0;
                if (la.Count > 0)
                {
                ...
                var la2 = MainOrderController.OrderListSalesStoreExcel(ac.ID, stype, search, fd, td);

                double or_PriceVND = 0;
                ...
                if (la2.Count > 0)
                {
                    foreach (var o in la2)
                    {
                        if (o.TQVNWeight.ToFloat(0) > 0)
                            Weight += Convert.ToDouble(o.TQVNWeight);
                        if (o.PriceVND.ToFloat(0) > 0)
                            or_PriceVND += Convert.ToDouble(o.PriceVND);
                ...
                HoaHong = Math.Round(or_PriceVND - or_FeeReal);
                ...
                lblFeeHoaHong.Text = string.Format("{0:N0}", HoaHong) + " VNĐ";
                lblPriceVND.Text = string.Format("{0:N0}", or_PriceVND) + " VNĐ";
                lblCountOrder.Text = la2.Count.ToString() + " Đơn";
                lblTotalWeight.Text = Weight.ToString() + " Kg";
            }
        }
        ...
        protected void btnFilter_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("Report-Sales?stype=" + stype + "&s=" + searchname + "&fd=" + fd + "&td=" + td + "");
        }

        public void pagingall(List<ReportOrderSales_Result> acs, int total)
        {
            int PageSize = 20;
            if (total > 0)
            {
                ...
                for (int i = 0; i < acs.Count; i++)
                {
                    ...
                    hcm.Append("<td>" + string.Format("{0:N0}", Convert.ToDouble(item.PriceVND) - Convert.ToDouble(item.TotalPriceReal)) + "</td>");
                    hcm.Append("<td>" + string.Format("{0:N0}", Convert.ToDouble(item.TQVNWeight)) + "</td>");
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            if (ac != null)
            {
                ...
                var la = MainOrderController.OrderListSalesStoreExcel(ac.ID, stype, search, fd, td);
